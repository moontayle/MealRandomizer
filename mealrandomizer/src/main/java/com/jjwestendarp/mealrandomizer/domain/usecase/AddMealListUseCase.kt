...
class AddMealListUseCase(private val mealsRepository: MealsRepository) : BaseUseCase.Async<Unit, List<Meal>> {
    override suspend fun run(params: List<Meal>): Either<Failure, Unit> {
        return Either.Success(mealsRepository.insertMeals(params))
    }
}